...
## SPQ data export
Run `spq_json_to_excel.py` to extract the Schizotypal Personality Questionnaire (SPQ) values from the JATOS results (per participant) and save them as `spq_data.xlsl` for later merging with the experiment data.
```sh
...
1. `03_modelling_experiment_and_data.Rmd`: Creates the 'mini' (60 trials), 'midi' (120 trials), and 'maxi' (200) datasets from the preprocessed data. 
...
3. `05_model_3dir.Rmd`: Runs the Baysian direct-effects model in Stan separately for 'mini', 'midi', and 'maxi' and saves the fitted models.
### Input
The script expects:
